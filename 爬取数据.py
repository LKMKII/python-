...
def get_content(html):
    ...
    text = text[text.index('=') + 1:-2]  # 改var data=将其变为json数据
    jd = json.loads(text)
    ...
    for day in li:  # 遍历找到的每一个li
        if i < 7 and i > 0:
            ...
            if inf[1].find('span') is None:  # 有时天气预报可能没有最高气温（后面再运行又有了（））
                tem_high = None
            else:
...
def main():
    ...
    url1 = 'http://www.weather.com.cn/weather/101280901.shtml'  # 7天天气中国天气网
    url2 = 'http://www.weather.com.cn/weather15d/101280901.shtml'  # 8-15天天气中国天气网
    ...
    # print(data)
    write_to_csv('ZQ-14d.csv', data14, 14)  # 保存为csv文件
    write_to_csv('ZQ-1d.csv', data1, 1)
